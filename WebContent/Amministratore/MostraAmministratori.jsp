...
<body class="is-preload">
	...
	%>
	<%
		Vector<Amministratore> admins = (Vector<Amministratore>) request.getAttribute("admins");
	...
	<!-- Heading -->
	<div id="heading">
		<h1><%=session.getAttribute("titolo")%></h1>
	</div>
		...
		<div class="inner">
			...
			<section>
				...
				<table class="alt">
					...
					<tbody>
						...
						<%
							...
							%>
						<tr>
							<td><%=a.getId_amministratore()%></td>
							<td><%=a.getPass()%></td>
							<td><a onclick="return confirm('Confermi?');"
								href="/EliminaAdmin?id=<%=a.getId_amministratore()%>">Elimina</a></td>
						</tr>
						<%
								}
							%>
				...
				<ul class="actions fit">
					<li><a onclick="newPw()" class="button primary fit small">Nuovo
							Amministratore</a></li>
				</ul>
	...
	<script>
		function newPw() {
			...
			xmlHttpReq.open('POST', "/NuovoAdmin", true);
			xmlHttpReq.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
			xmlHttpReq.send(prompt("Inserisci password", ""));
			window.location.href = "/RichiediGestisciAdimn";
